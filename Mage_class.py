...
class Knight(PlayerBase):
    ...
    def throw_bottle(self):
        ...
        velocity = 50  # начальная скорость

        # Создаём бутылку с учётом стороны броска
        if self.facing_right:
            bottle = Bottle(start_x + 10, start_y, velocity)  # бросок вправо
        else:
            bottle = Bottle(start_x - 10, start_y, velocity)  # бросок влево
...
class Bottle(arcade.Sprite):
    def __init__(self, start_x, start_y, velocity):
        ...
        self.gravity = -9.8  # гравитация (можно уменьшить для игры)
        self.time = 0.0
        self.is_active = True
        FIXED_ANGLE_DEGREES = 25  # угол броска (можно подобрать)
    ...
    def update(self, delta_time):
        ...
        self.center_y += (self.vy * self.time + 0.5 * self.gravity * self.time**2)

        if self.center_y < 50:  # столкновение с землёй
            self.explode()
